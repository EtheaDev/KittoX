...
begin
  Synchronize(
    ...
    begin
      ...
      // (e.g. auto-built/inline views registered by TreePanel as dynamic
      // and also created by FindObjectByNode as nonpersistent).
      LFreedObjects := TList<TKMetadata>.Create;
      try
        for LObject in FNonpersistentObjects.Values do
        begin
          if not LFreedObjects.Contains(LObject) then
          begin
            LFreedObjects.Add(LObject);
            LObject.Free;
          end;
        end;
...
begin
  ...
  Synchronize(
    ...
    begin
      ...
      LResult := System.SysUtils.FindFirst(IncludeTrailingPathDelimiter(FPath) + '*.yaml', faNormal, LSearchRec);
      while LResult = 0 do
      begin
        ...
        LResult := System.SysUtils.FindNext(LSearchRec);
      end;
