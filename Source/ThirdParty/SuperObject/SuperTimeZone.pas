unit SuperTimeZone; // usata solo in Delphi 12
...
{$if defined(VER230) or defined(VER240)  or defined(VER250) or
     ...
     defined(VER290) or defined(VER310)  or defined(VER310) or defined(VER320)}
  {$define VER210ORGREATER}
...
type
  ...
  private
    FName: SOString;
    FCurrentTimeZone: SOString;
    FTimeZoneRegistry: record
      Exists: Boolean;
  ...
  public
    ...
    procedure ReadTimeZoneRegistry;
...
begin
  inherited Create;
  FName := TimeZoneName;
  FCurrentTimeZone := GetCurrentTimeZone;
  ReadTimeZoneRegistry;
end;
...
procedure TSuperTimeZone.ReadTimeZoneRegistry;
var
  KeyName: SOString;
  RegTZI: TRegistryTZI;
begin
  FillChar(FTimeZoneRegistry.TZI, SizeOf(FTimeZoneRegistry.TZI), 0);
  FTimeZoneRegistry.DaylightDisabled := False;
  FTimeZoneRegistry.DynamicDST := False;
  FTimeZoneRegistry.FirstYear := 0;
  FTimeZoneRegistry.LastYear := 0;
  KeyName := TZ_KEY + Name;
  with TRegistry.Create do
  try
    RootKey := HKEY_LOCAL_MACHINE;
    FTimeZoneRegistry.Exists := KeyExists(KeyName);
    if OpenKeyReadOnly(KeyName) then
    begin
      if ValueExists('IsObsolete') then
        FTimeZoneRegistry.DaylightDisabled := ReadBool('IsObsolete');
      CloseKey;
    end;
    if OpenKeyReadOnly(KeyName + '\Dynamic DST') then
    begin
      FTimeZoneRegistry.DynamicDST := True;
      FTimeZoneRegistry.FirstYear := ReadInteger('FirstEntry');
      FTimeZoneRegistry.LastYear  := ReadInteger('LastEntry');
      CloseKey;
    end
    else
    begin
      if OpenKeyReadOnly(KeyName) then
        ...
        FTimeZoneRegistry.TZI.Bias         := RegTZI.Bias;
        FTimeZoneRegistry.TZI.StandardDate := RegTZI.StandardChangeTime;
        FTimeZoneRegistry.TZI.StandardBias := RegTZI.StandardBias;
        FTimeZoneRegistry.TZI.DaylightDate := RegTZI.DaylightChangeTime;
        FTimeZoneRegistry.TZI.DaylightBias := RegTZI.DaylightBias;
        CloseKey;
      end;
  finally
    Free;
...
begin
  ...
  if not FTimeZoneRegistry.Exists then
  begin
    Result := False;
    Exit;
  end;
  ...
  ChangeYear := 0;
  if FTimeZoneRegistry.DynamicDST then
  begin
    FirstYear := FTimeZoneRegistry.FirstYear;
    LastYear  := FTimeZoneRegistry.LastYear;
  ...
  if ChangeYear = 0 then
  begin
    RegTZI.Bias := FTimeZoneRegistry.TZI.Bias;
    RegTZI.StandardChangeTime := FTimeZoneRegistry.TZI.StandardDate;
    RegTZI.StandardBias := FTimeZoneRegistry.TZI.StandardBias;
    RegTZI.DaylightChangeTime := FTimeZoneRegistry.TZI.DaylightDate;
    RegTZI.DaylightBias := FTimeZoneRegistry.TZI.DaylightBias;
  end;
...
begin
  llTime := 0;
  if (not FTimeZoneRegistry.DaylightDisabled) and (pTZinfo^.DaylightDate.wMonth <> 0) then
  begin
