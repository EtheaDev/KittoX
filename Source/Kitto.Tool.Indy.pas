...
/// <summary>
///  Email sending tool controller via Indy SMTP for KittoX.
///  Replaces Kitto.Ext.IndyTools.
/// </summary>
unit Kitto.Tool.Indy;
...
begin
  ...
  try
    ...
    try
      ...
      if LHTMLBody <> '' then
      begin
        LHTMLBody := StringReplace(LHTMLBody,'','&euro;',[rfReplaceAll]);
        // HTML body present.
        if LBody = '' then
        begin
          // HTML only.
...
initialization
  TKXControllerRegistry.Instance.RegisterClass('SendEmail', TSendEmailToolController);

finalization
  TKXControllerRegistry.Instance.UnregisterClass('SendEmail');
