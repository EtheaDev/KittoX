...
///  AVisibility is one of the SW_* constants defined in the unit WinApi.Windows.
...
begin
  ...
  if not WinApi.Windows.DeleteFile(PChar(ASourceFileName)) then
  begin
...
begin
  ...
  if not WinApi.Windows.CopyFile(PChar(ASourceFileName), PChar(ADestinationFileName), False) then
  begin
    LastError := GetLastError();
    if LastError <> 0 then
      LErrorMsg := Format(SOSError, [LastError, SysErrorMessage(LastError)])
