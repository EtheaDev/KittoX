...
var
  LVarType: TVarType;
begin
  LVarType := VarType(AVariant) and varTypeMask;
  Result := ((LVarType = varString) or (LVarType = varUString) or (LVarType = varOleStr))
    and (VarToStr(AVariant) = '');
end;

function EFVarIsEmpty(const AVariant: Variant): Boolean;
begin
  Result := VarIsNull(AVariant) or VarIsEmpty(AVariant) or EFVarIsEmptyString(AVariant);
end;
