...
begin
  // For DLLs (ISAPI), HInstance points to the DLL module.
  // For EXEs, HInstance = MainInstance, same as ParamStr(0).
  Result := ExtractFilePath(GetModuleName(HInstance));
  // Strip \\?\ long path prefix that Windows may add for ISAPI DLLs
...
begin
  ...
  if AString.Contains('%APP_PATH%') then
    ExpandMacros(AString, '%APP_PATH%', GetModulePath);
  if AString.Contains('%APP_NAME%') then
    ExpandMacros(AString, '%APP_NAME%', GetModuleName(HInstance));
