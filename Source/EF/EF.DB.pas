...
procedure TEFDBConnection.StartTransaction;
begin
  if not IsOpen then
    Open;
...
procedure TEFDBConnection.CommitTransaction;
begin
...
procedure TEFDBConnection.RollbackTransaction;
begin
  if (FTransactionCount = 1) then
  begin
    FTransactionCount := 0;
...
begin
  ...
  if Trunc(ADateTimeValue) = ADateTimeValue then
    Result := System.SysUtils.FormatDateTime('yyyy''-''mm''-''dd', ADateTimeValue)
  else
...
begin
  //Check if the value is only a date
  if Trunc(ADateTimeValue) = ADateTimeValue then
    Result := System.SysUtils.FormatDateTime('yyyymmdd', ADateTimeValue)
  else
    Result := System.SysUtils.FormatDateTime('yyyy''-''mm''-''ddThh'':''mm'':''ss''.''zzz', ADateTimeValue);
end;
...
begin
  //Check if the value is only a date
  if Trunc(ADateTimeValue) = ADateTimeValue then
    Result := Format('to_date(''%s'', ''yyyymmdd'')', [System.SysUtils.FormatDateTime('yyyymmdd', ADateTimeValue)])
  else
    Result := Format('to_date(''%s'', ''yyyy/mm/dd hh24:mi:ss'')', [System.SysUtils.FormatDateTime('yyyymmdd hh:mm:ss', ADateTimeValue)]);
end;
...
var
  ...
  I: Integer;
begin
  // Strip driver-level prefixes like [FireDAC][Phys][ODBC][Microsoft]...[SQL Server]
  // keeping only the user-readable message after the last ']'.
  LUserError := ASQLError;
  if (LUserError <> '') and (LUserError[1] = '[') then
  begin
    I := LUserError.LastIndexOf(']');
    if I >= 0 then
      LUserError := Trim(Copy(LUserError, I + 2, MaxInt));
  end;
  if LUserError = '' then
    LUserError := ASQLError;
  ...
  {$IFNDEF DEBUG}
  LErrorMsg := Format(LMessage, [LUserError, FGUIDError]);
  {$ENDIF}
  FSQLError := LUserError;
