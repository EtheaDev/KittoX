...
type
  ...
  public
    [YamlSubNode('Connection', TEFDBFDConnectionConfig, 'FireDAC connection parameters')]
    property ConnectionConfig: TEFDBFDConnectionConfig read GetConnectionConfig;
  end;
...
var
  ...
  LDefName: string;
  LPoolMaxItems: Integer;
  LPoolParams: TStrings;
begin
  inherited;
...
procedure TEFDBFDConnection.InternalStartTransaction;
begin
  FConnection.StartTransaction;
end;

procedure TEFDBFDConnection.InternalCommitTransaction;
begin
  FConnection.Commit;
end;

procedure TEFDBFDConnection.InternalRollbackTransaction;
begin
...
begin
  ...
  // FireDAC's mkTableFields does NOT surface the native field comment
  // for any of the supported engines. We fetch them separately with
  // a per-driver SQL query so the values land in
  // TEFDBColumnInfo.Description and propagate to Metadata.DisplayLabel
  // when the model is generated by the KIDEX wizard or by the MCP
  // tool models_create_from_db.
  FetchTableColumnDescriptions(ATable);
end;

procedure TEFDBFDInfo.FetchTableColumnDescriptions(const ATable: TEFDBTableInfo);
var
  LDriverID: string;
  LQuery: TFDQuery;
  LSQL: string;
  ...
  LColumn: TEFDBColumnInfo;
begin
  // Get the FireDAC driver short name to dispatch the right query.
  // Connection.DriverName returns 'MSSQL', 'PG', 'FB', 'IB', 'MySQL',
  // 'Ora' (Oracle), 'SQLite', etc.
  LDriverID := FConnection.DriverName;
  if LDriverID = '' then
    Exit;

  // Build the per-driver SQL. Each statement returns two columns:
  // (column_name, description). When the engine has no native
  // comment storage, or we don't (yet) support it, we exit silently.
  LSQL := '';
  if SameText(LDriverID, 'MSSQL') then
    LSQL :=
      ...
      '   AND ep.class = 1 ' +  // OBJECT_OR_COLUMN class
      'WHERE c.object_id = OBJECT_ID(' + QuotedStr(ATable.Name) + ')'
  else if SameText(LDriverID, 'PG') then
    LSQL :=
      ...
      'WHERE a.attrelid = ' + QuotedStr(ATable.Name) + '::regclass ' +
      '  AND a.attnum > 0 AND NOT a.attisdropped'
  else if SameText(LDriverID, 'FB') or SameText(LDriverID, 'IB') then
    // Firebird stores the comment in RDB$DESCRIPTION (a BLOB SUB_TYPE
    // TEXT). FireDAC fetches BLOBs transparently as strings on the
    // dataset side.
    LSQL :=
      ...
      QuotedStr(UpperCase(ATable.Name)) +
    ...
    LSQL :=
      ...
      '  AND TABLE_NAME = ' + QuotedStr(ATable.Name) +
      '  AND COLUMN_COMMENT <> '''''
  else if SameText(LDriverID, 'Ora') then
    LSQL :=
      'SELECT COLUMN_NAME AS column_name, ' +
      ...
      'WHERE TABLE_NAME = ' + QuotedStr(UpperCase(ATable.Name)) +
  ...
  try
    ...
    try
      ...
      // A failing comment query must not break model creation. The
      // table may be a view, the catalog may be locked, the DB user
      // may lack metadata permissions -- swallow the error and leave
      // descriptions empty.
      Exit;
    end;
    ...
    begin
      ...
      LDescription := LQuery.FieldByName('description').AsString;
...
function TEFDBFDAdapter.GetConnectionConfig: TEFDBFDConnectionConfig;
begin
