...
begin
  inherited;
  if not TKAuthenticator.Current.IsAuthenticated then
  begin
    TKWebApplication.Current.Toast(_('An e-mail with login instructions has been sent'));
  end;
end;
...
begin
  ...
  begin
    ...
    if TKConfig.Instance.Config.GetBoolean('IsClearPassword') then
      ARecord.FieldByName('PASSWORD_HASH').AsString := LPassword
    else if ContainsText(TKAuthenticator.Current.GetClassId,'BCrypt') then
      ARecord.FieldByName('PASSWORD_HASH').AsString := GetBCryptedString(LPassword)
    else
      ARecord.FieldByName('PASSWORD_HASH').AsString := GetStringHash(LPassword);
