...
function IsNumber(const strWord : string): Boolean;
var
  i: Integer;
  CurChar: Char;
begin
  ...
  begin
    Result := CharInSet(CurChar, ['0'..'9']);
    for i := 1 to length(strWord) - 1 do
    begin
      CurChar := strWord[i];
      if not CharInSet(CurChar, ['0'..'9','.']) then
      begin
        Result := False;
        Break;
      end
...
*)
procedure TYAMLHighlighter.Tokenize(StartClass: TOTALineClass; LineBuf: POTAEdChar;
  LineBufLen: TOTALineSize; HighlightCodes: POTASyntaxCode);
...
begin
  ...
  For i := 0 To LineBufLen - 1 Do
    begin
      ...
      begin
        CurChar := Char(LineBuf[i]);
        if CharInSet(CurChar, ['#']) Or (BlockType In [btComment]) then
        begin
          Codes[i] := AnsiChar(atComment);
          BlockType := btComment;
        end
        else if CharInSet(CurChar, [':']) then
        begin
        ...
        else if CharInSet(CurChar, ['"']) or (BlockType In [btString]) then
        begin
          Codes[i] := AnsiChar(atString);
          BlockType := btString;
        end
        else if CharInSet(CurChar, strChars) then
        begin
          Codes[i] := AnsiChar(atString);
        ...
        else if CharInSet(CurChar, strNumbers) or CharInSet(CurChar, ['-','.']) or (BlockType In [btNumber]) then
        begin
          Codes[i] := AnsiChar(atNumber);
          BlockType := btNumber;
        end
        else if CharInSet(CurChar, strSymbols) then
          Codes[i] := AnsiChar(atString)
        else if CharInSet(CurChar, strBlock) then
          isMultipleLine := True;
...
**)
function TYAMLHighlighter.TokenizeLineClass(StartClass: TOTALineClass;
