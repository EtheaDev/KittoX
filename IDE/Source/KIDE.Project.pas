...
begin
  // Clear the singleton-redirection hooks installed by RefreshConfig
  // so that any code running between projects does NOT see a dangling
  // reference to the freed project's FConfig. The web runtime
  // (TKWebApplication) does the same on Destroy.
...
begin
  ...
  // Redirect TKConfig.Instance singleton to the open project's config.
  // Without this, class-level methods like TKConfig.DatabaseFor (used by
  // the model wizard's PAGE_DB → PAGE_SELECT transition and by the MCP
  // tool models_create_from_db) read the global FInstance — which in
  // KIDEX is the TKideConfig singleton (KIDEX's own Bin/Config.yaml),
  // not the project's. Result was 'EEFError: Nodo Databases/Main/
  // Connection non trovato'. The web runtime installs the same hook
  // (TKWebApplication class constructor) for the same reason.
